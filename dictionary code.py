...
def add_to_dict(a_dict,key,value=""):
 if type(a_dict) is not dict: 
   print("You need send a dictionary. You sent: ",type(a_dict))
 elif value =="":
    print("You need to send a word and a definition.정의해줘")
 elif key in a_dict:
   print(f"{key} is already on thee dictionary. Won't add.")
 else :
   a_dict[key] = value
...
def get_from_dict(a_dict,key=""):
  if type(a_dict) is not dict: 
   ...
   print(f"{key}: The source of life.")
